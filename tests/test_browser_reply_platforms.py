...
class TestBrowserReplyPlatformScripts(unittest.TestCase):
    def test_bilibili_uses_shadow_dom_reply_editor(self):
        ...
        self.assertIn("bilibili_nested_reply_expand", fill)
        self.assertIn("bili-comment-reply-renderer", fill)
        self.assertIn("walkShadow", fill)
        self.assertIn("collectTargets", fill)
        self.assertIn("nested:matchedTarget.kind === 'reply'", fill)
    ...
    def test_weibo_uses_comment_icon_and_dedicated_reply_modal(self):
        ...
        self.assertIn("document.scrollingElement", fill)
        self.assertIn("window.scrollTo", fill)
        self.assertIn("weibo_nested_reply_expand", fill)
        self.assertIn(".con2 .con1", fill)
        self.assertIn("before_initial_scan", fill)
        self.assertIn("after_scroll", fill)
    ...
    def test_existing_platforms_keep_generic_reply_path(self):
        ...
        # 小红书楼中楼默认折叠，回复定位前必须展开并等待子评论挂载，
        # 否则只扫描一级评论会漏掉实际目标。
        xhs_fill = _fill_script(_target("xhs"), "测试回复")
        self.assertIn("const isXhs =", xhs_fill)
        self.assertIn("xhs_nested_reply_expand", xhs_fill)
        self.assertIn("展开 N 条回复", xhs_fill)
        self.assertIn("isXhsExpandLabel", xhs_fill)
        self.assertIn("getAttribute?.('aria-label')", xhs_fill)
        self.assertIn("const effectiveExpandClickLimit = isXhs ? 48 : expandClickLimit", xhs_fill)
        self.assertIn("await sleep(isXhs ? 1000 : 700)", xhs_fill)
        ...
        self.assertIn("if (!isDouyin && !isXhs)", video_fill)
