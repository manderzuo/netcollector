...
class CdpSession:
    ...
    async def cmd(self, method: str, params: dict = None, session_id: str = None,
        ...
        if self.ws is None:
            self._trace.emit("command_rejected", method=method, reason="not_connected")
        ...
        logged_params = dict(params or {})
        if method == "Runtime.evaluate" and "expression" in logged_params:
            expression = str(logged_params.get("expression") or "")
            logged_params["expression_sha256"] = hashlib.sha256(
                expression.encode("utf-8", errors="replace")
            ).hexdigest()
            logged_params["expression_length"] = len(expression)
            logged_params["expression_preview"] = expression[:800]
            logged_params.pop("expression", None)
        self._trace.emit(
            "command_started",
        ...
        try:
            ...
            self._trace.emit(
                ...
                result_summary=(
                    {"has_exception": bool(isinstance(result, dict) and result.get("exceptionDetails")),
                     "result_keys": list(result.keys()) if isinstance(result, dict) else []}
                    if method == "Runtime.evaluate" else str(result)[:1200]
                ),
            )
            return result
        except Exception as exc:
            duration_ms = round((time.perf_counter() - started) * 1000, 2)
            # Network 响应体可能在 responseReceived 到 getResponseBody 之间
            # 被 Chromium 回收。这是旁观抓包时的正常竞态，调用方本来就会
            # 跳过该响应；不应伪装成高优先级 CDP 故障并反复刷屏。
            if (
    ...
    async def eval(self, expression: str, session_id: str,
                   return_by_value: bool = True, *, timeout: float = None):
