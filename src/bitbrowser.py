...
class BitBrowserClient:
    ...
    def __init__(self, base_url: str = "http://127.0.0.1:54345", timeout: float = 20):
        ...
        self._trace = DebugTrace("bitbrowser_api")
    ...
    def _post(self, path: str, body: Optional[dict] = None) -> Any:
        ...
        try:
            ...
            self._trace.exception(
                "api_request_failed", exc, method="POST", path=path,
                duration_ms=round((time.perf_counter() - started) * 1000, 2),
                failure_stage="http", http_status=exc.code,
            )
        ...
        try:
            ...
            self._trace.exception(
                "api_response_parse_failed", exc, method="POST", path=path,
                duration_ms=round((time.perf_counter() - started) * 1000, 2),
        ...
        self._trace.emit(
            ...
            data_type=type(obj.get("data")).__name__ if isinstance(obj, dict) else type(obj).__name__,
            data_count=(len(obj.get("data")) if isinstance(obj, dict)
                        and isinstance(obj.get("data"), (dict, list, tuple)) else None),
            message=obj.get("msg") if isinstance(obj, dict) else None,
        )
        if not obj.get("success"):
            msg = obj.get("msg") or "未知错误"
            self._trace.emit(
                "api_business_failed", method="POST", path=path,
                duration_ms=round((time.perf_counter() - started) * 1000, 2),
                message=msg,
            )
        ...
        self._trace.emit(
            "api_request_completed", method="POST", path=path,
            duration_ms=round((time.perf_counter() - started) * 1000, 2),
        )
        return obj.get("data")
